...
size_t mc_saveData(
  ...
  saveBufferToFile(
    thisParallelProgram,
    thisParallelProgram.m_dataPool[1].first,
    thisParallelProgram.m_dataPool[1].second,
    "result_mc_posts");
...
void pirk::initializeMonteCarlo(const std::shared_ptr<pfacesKernelLaunchState>& spLaunchState) {
  ...
  /* some constants */
  std::vector<std::string> argNames_initialize = {
    "initial_state", "final_state", "input", "k0", "k1", "k2", "k3", "tmp", "t", "rands1", "rands2"};
  std::vector<std::string> argNames_integrate = {
    "initial_state", "final_state", "input", "k0", "k1", "k2", "k3", "tmp", "t"};
...
void pirk::configureParallelProgramMonteCarlo(pfacesParallelProgram& parallelProgram) {
  ...
  cl::NDRange ndUniversalRangeStateDim(
    pfacesBigInt::getPrimitiveValue(nsamples), pfacesBigInt::getPrimitiveValue(states_dim));
  ...
  std::pair<cl::NDRange, cl::NDRange> stateDimProcessRangeAndOffset =
    parallelAdvisor.getProcessNDRangeAndOffset(ndUniversalRangeStateDim);
  ...
  jobsPerDev_mc_initialize = parallelAdvisor.distributeJob(
    *this,
    ...
    parallelProgram
      .m_isFixedJobDistribution, /* whether or not to use a fixed job distribution, or to tune automatically */
    parallelProgram.m_fixedJobDistribution, /* the fixed distribution, if one is being used */
    true,
    false,
    false /* some additional flags */
  );
  ...
  memReport = allocateMemory(
    dataPool,
    parallelProgram.getMachine(),
    parallelProgram.getTargetDevicesIndicies(),
    pfacesUtils::oclGetRangeVolume(ndProcessRangeStateDim),
    false);
  ...
  jobReadAllData
    ->setKernelFunctionIdx(   /* this function tells the instruction where to read the data from */
                           0, /* index of the kernel function we're to read data from (0 in this case, since matrixAdd
                                 is index 0 */
  ...
  instrHostSideFuncGenerateRandomNumbers->setAsHostFunction(
    mc_hostFunctionGenerateRandomNumbers, "mc_hostFunctionGenerateRandomNumbers");
